...
export default function Dashboard(){
  ...
  const [csvInfo,setCsvInfo]=useState({ total:0, tags:{} }) // summary of CSV
  const [tagMap,setTagMap]=useState({})          // { TagName: [TICKER, ...] }

  function sanitizeTicker(s){
    return String(s||'').toUpperCase().replace(/[^A-Z0-9.-]/g,'').slice(1,7).length
  ...
  function handleCSV(e){
    ...
    Papa.parse(file, {
      ...
      complete: (res)=>{
        ...
        const totalTickers = new Set(mapped.map(r=>r.ticker)).size
        setCsvInfo({ total: totalTickers, tags })

        // also prefill the paste box with unique tickers from CSV
        setTickers(Object.keys([...mapped.reduce((s,r)=>s.add(r.ticker), new Set())]).join(', '))
      }
    })
  }
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 3fr',gap:8, marginTop:8}}>
            {Object.entries(csvInfo.tags).map(([tag,arr])=>(
              <div key={tag} style={{borderTop:'1px solid #f2f2f2',paddingTop:6}}>
                ...
                <div style={{fontSize:12,opacity:0.9}}>{arr.join(', ')}</div>
              </div>
            ))}
          </div>
        </div>
